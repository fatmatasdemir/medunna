...
//                System.out.println(i++);
    ...
    public static List<String> returnReadRoomMessages(String filePath){
        List<String>all = new ArrayList<>();
        try(BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            StringBuilder sb = new StringBuilder();
            String line = br.readLine();
            ...
            int i = 0;
            while (line != null) {
               Messages messagesPojo = new Messages();
                messagesPojo.setSubject(line.split(",")[0]);
                sb.append(System.lineSeparator());
                line = br.readLine();

//                System.out.println(i++);

                all.add(messagesPojo.getSubject());
            }
        }catch (Exception e){
            e.printStackTrace();
        }
